...
import { Input } from '@/components/ui/input'; // Asumsi ini ada
import { Button } from '@/components/ui/button'; // Asumsi ini ada
import { Skeleton } from '@/components/ui/skeleton'; // Asumsi ini ada
import { 
    Search, 
    DownloadCloud, 
    Trophy, 
    ...
    Filter, 
    X, 
    ZoomIn, // Ikon baru untuk View // Ikon baru untuk Loading
    FileX2, // Ikon baru untuk Not Found
    ChevronDown
} from 'lucide-react';
import Lightbox from 'yet-another-react-lightbox';
import 'yet-another-react-lightbox/styles.css';
import Captions from "yet-another-react-lightbox/plugins/captions"; // Plugin untuk caption
...
const ResultCard = ({ item, onImageClick, onDownloadClick }: { item: HasilAkhir; onImageClick: (url: string) => void; onDownloadClick: (url: string, filename: string) => void; }) => {
    ...
    const filename = `hasil-${item.mata_lomba}-${item.nama_sekolah}.jpg`; // Asumsi JPG
    ...
    return (
        <motion.article 
            ...
            layout
        >
            <div className="relative aspect-[4/5] w-full"> {/* Ubah aspect ratio agar lebih tinggi */}
                <Image
                    src={imageUrl}
                ...
                >
                    ...
                    <Button
                        ...
                        title="Unduh Gambar"
                        onClick={(e) => {
                            e.stopPropagation(); // Mencegah lightbox terbuka saat download
...
const HasilAkhirPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select('*')
          .order('mata_lomba')
          .order('nomor_urut'); // Urutkan berdasarkan lomba dan nomor
  ...
  const filteredData = useMemo(() => {
    return hasilAkhirData.filter((item) => {
        const itemMataLomba = item.mata_lomba.replace(/_/g, ' '); // Normalisasi nama lomba dari DB
  ...
  }, [hasilAkhirData, selectedLomba, searchQuery]);
  ...
  // Handle Image Click (Buka Lightbox)
  ...
  // Custom styled select (karena select bawaan sulit di-style)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-[#4A1313] to-[#1A0D0D] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
        ...
        {/* Content Section */}
        <motion.div
            key={selectedLomba + searchQuery} // Ganti key agar animasi berjalan saat filter berubah
        ...
        <Lightbox
            ...
            styles={{
                ...
                captionsTitle: { color: "#FFBF00" }, // Amber color
            }}
            controller={{ 
